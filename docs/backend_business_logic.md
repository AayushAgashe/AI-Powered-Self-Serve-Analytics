# OPUS BI: Backend Business Logic & Service Reference

This document provides a highly detailed, service-by-service explanation of the **OPUS BI Backend Engine** (FastAPI + SQL + LLM RAG + DuckDB pipeline). It outlines exactly how natural language queries are processed, secured, executed, visualized, and stored, documenting the inputs, return values, schema structures, and database persistence layouts.
